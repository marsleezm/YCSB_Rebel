...
Output=$2/output
ThroughOut=$2/$Time
head -$NumNodes ./scripts/allnodes > ./to_run
...
then
    ...
    echo marco | sudo -S timeout ${Duration}s cassandra-stress user profile=./rebel.yaml ops\(timeline=1\) n=50000000  -node file=./to_run -rate threads=64 > $Output 2>&1 
else
