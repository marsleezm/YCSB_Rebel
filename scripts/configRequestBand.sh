...
C0="sudo tc qdisc del root dev eth4" 
C1="sudo tc qdisc add dev eth4 root handle 1: htb default 10" 
...
C2="sudo tc class add dev eth4 parent 1: classid 1:1 htb rate 200mbps ceil 200mbps" 
C4="sudo tc qdisc add dev eth4 parent 1:1 handle 10: sfq perturb 10"
#C5="sudo iptables -A OUTPUT -t mangle -p tcp --sport 9042 -j MARK --set-mark 10"
C5="sudo iptables -A OUTPUT -t mangle -p tcp --sport 7000 -j MARK --set-mark 10"
C6="sudo tc filter add dev eth4 protocol ip parent 1:0 prio 1 handle 10 fw flowid 1:1"
